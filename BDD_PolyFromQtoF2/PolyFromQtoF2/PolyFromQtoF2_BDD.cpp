...
BDD getTermAsBDD(Cudd mgr,polynomial_t poly);
tuple<polynomial_t, polynomial_t> variableDivision(polynomial_t poly, tuple<string, int> variable);
BDD polyFromQtoF2(Cudd mgr,polynomial_t poly,int list_counter);
...
bool bIsNegative;
int counter=0;
std::unordered_map<string,BDD> uvarMap;
int var_index;
...
int main (int argc, char* argv[]) 
{
    ...
    for(std::map<string, polynomial_t>::iterator iterMap = gmBasis.begin(); iterMap != gmBasis.end(); ++iterMap)
    {
        ...
        BDD return_poly = polyFromQtoF2(mgr,polynomial,counter);
    ...
    // for(int mono_iter=0;mono_iter<32;mono_iter++)
    // {
...
*/
   ...
   if(p_poly.size()==0)
   {
    ...
    auto it1 = uvarMap.find(get<string>(variable));
    if (it1 == uvarMap.end()) {
        // If not, create a new variable and add it to the map
        BDD var = mgr.bddVar();
        var_index = var.NodeReadIndex();
        uvarMap[get<string>(variable)] = var;
    } else {
        var_index = it1->second.NodeReadIndex();
    }
    ...
    //Recurrsion starts
    //cout << "Recurse g:" << endl;
    BDD g_tilda = polyFromQtoF2(mgr,g_poly, list_counter);
    //cout<<"g_tilda" << sPolyAsString(g_tilda)<<endl;
    //cout << "Recurse p:" << endl;
    BDD p_tilda = polyFromQtoF2(mgr,p_poly, list_counter);
    ...
    BDD ftilda=((p_tilda ^ g_tilda)* uvarMap[get<string>(variable)])^g_tilda;
    std::vector<BDD> nodes;
    //nodes.push_back(x);
    //nodes.push_back(y);
    nodes.push_back(ftilda);
    //nodes.push_back(tmp);
    FILE *fp = fopen("ftilda.dot", "w");
...
BDD getTermAsBDD(Cudd mgr,polynomial_t poly)
{
    ...
    for(int i=0;i<term_temp.monomial.size();i++){
        ...
        {
            varNames.push_back(get<string>(term_temp.monomial[i]));
        }
    }
    for(int i=0;i<varNames.size();i++)
    {
        auto it = uvarMap.find(varNames[i]);
        if (it == uvarMap.end()) {
        // If not, create a new variable and add it to the map
        BDD var = mgr.bddVar();
        var_index = var.NodeReadIndex();
        uvarMap[varNames[i]] = var;
        } 
        else {
        var_index = it->second.NodeReadIndex();
        }
        tilda=tilda*uvarMap[varNames[i]];
    }
    return tilda;
}
