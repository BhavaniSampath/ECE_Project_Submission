...
*/
    ...
    for(std::map<string, polynomial_t>::iterator iterMap = gmBasis.begin(); iterMap != gmBasis.end(); ++iterMap)
    {
        ...
        // tuple<polynomial_t, polynomial_t> divTuple = variableDivision(polynomial, variable_list[5]);
...
*/
   ...
   polynomial_t p_poly= hPlusg(h_poly,g_poly);
   ...
   if(p_poly.size()==0)
   {
    ...
    cout<<"F2 ppoly:"<< sPolyAsString(ftilde)<<endl;
...
polynomial_t hPlusg(polynomial_t h,polynomial_t g)
{
    ...
    for(iter=h.begin();iter != h.end(); ++iter)
    {
        term_t term_h = *iter;
        p.push_back(term_h);
        polynomial_t::iterator iter1;
        for (iter1=g.begin();iter1!=g.end();++iter1)
        {   term_t term_g = *iter1;
            if((term_g.coefficient.numerator!=0) && (term_g.monomial!=term_h.monomial))
                {
                    p.push_back(term_g);
                }
